...
describe("security scanner", () => {
  it("builds an IDE firewall scan across agents, extensions, secrets, and MCP servers", async () => {
    ...
    expect(scan.extensions[0].accessLevel).toBe("high");
    expect(scan.extensions[0].capabilityTags).toEqual(expect.arrayContaining(["AI/agent", "terminal/commands"]));
    expect(scan.extensions[0].manifestEvidence.join(" ")).toContain("Run Shell Agent");
    ...
    expect(scan.findings.map((finding) => finding.id)).toContain("blast-radius:agent-deploy-secret-combination");
    expect(scan.categoryScores.mcpRisk).toBeLessThan(100);
    expect(scan.categoryScores.blastRadius).toBeLessThan(100);
    expect(scan.summary.highImpactConnections).toBeGreaterThan(0);
    expect(scan.connectionMap.nodes.length).toBeGreaterThan(0);
    expect(scan.connectionMap.edges.length).toBeGreaterThan(0);
    expect(scan.connectionMap.highRiskPaths.length).toBeGreaterThan(0);
