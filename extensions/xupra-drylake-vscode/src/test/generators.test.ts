...
describe("runbook generators", () => {
  it("renders RUNBOOK.md and phase prompts", () => {
    ...
    expect(prompt).toContain("You are executing phase 01-intake");
    expect(prompt).toContain("The user clicked Run Handoff. Treat that click as approval to execute this phase now.");
    expect(prompt).toContain("this Run Handoff invocation satisfies that approval for this phase's local execution.");
    expect(prompt).toContain("Do not stop for another planning approval");
    expect(prompt).not.toContain("Do not skip approval gates.");
  });
  ...
  it("uses active build-session provider for generated phase files without explicit agents", () => {
    ...
    const activeProvider = {
      ...
      providerLabel: "Xupra AI" as const,
    };
