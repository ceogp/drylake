...
import {
  refineRunbookArchitecturePrompt,
...
export async function POST(request: Request) {
  try {
    ...
    await recordRunbookPlanningUsage({
      ...
      promptKind: "planning_refine_architecture",
      promptText: parsed.data.prompt,
      metadata: {
        endpoint: "refine-architecture",
    ...
    const result = await refineRunbookArchitecturePrompt(parsed.data, { model: access.model });
    ...
    console.error(error);
    return internalError("Failed to refine DryLake runbook architecture");
  }
}
